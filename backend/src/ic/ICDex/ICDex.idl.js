...
module.exports = ({ IDL }) => {
  ...
  const IDOSetting = IDL.Record({
    'IDOSupplies' : IDL.Vec(
      IDL.Record({ 'supply' : Amount, 'price' : IDL.Nat })
    ),
  ...
  const CallType = IDL.Variant({
    ...
    'This' : IDL.Variant({
      'updatePoolLocalBalance' : IDL.Tuple(
        IDL.Opt(
          IDL.Variant({ 'add' : IDL.Nat, 'set' : IDL.Nat, 'sub' : IDL.Nat })
        ),
        IDL.Opt(
          IDL.Variant({ 'add' : IDL.Nat, 'set' : IDL.Nat, 'sub' : IDL.Nat })
        ),
      ...
      'sto_updateProOrder' : IDL.Tuple(
        ...
        IDL.Variant({
          'GridOrder' : IDL.Record({
            ...
            'spread' : IDL.Opt(
              IDL.Variant({ 'Geom' : Ppm__1, 'Arith' : Price__2 })
            ),
            'amount' : IDL.Opt(
              IDL.Variant({
  ...
  const GridOrder = IDL.Record({
    'setting' : GridSetting,
    'level1Filled' : IDL.Opt(
      IDL.Record({ 'buy1' : Amount__2, 'sell1' : Amount__2 })
    ),
    'filter' : IDL.Opt(
      IDL.Record({
        'gridTop' : Price__2,
  ...
  const BackupResponse = IDL.Variant({
    'icdex_keepingBalances' : IDL.Vec(
      IDL.Tuple(AccountId__3, KeepingBalance__1)
    ),
    ...
    'icdex_userProOrderList' : IDL.Vec(
      IDL.Tuple(AccountId__3, IDL.Vec(Soid__1))
    ),
  ...
  return IDL.Service({
    ...
    'IDO_setWhitelist' : IDL.Func(
        [IDL.Vec(IDL.Tuple(Address, Amount))],
        [],
        [],
      ),
    ...
    'latestFilled' : IDL.Func(
        ...
        [
          IDL.Vec(
            IDL.Tuple(Timestamp__5, Txid__1, OrderFilled__1, OrderSide__2)
          ),
        ],
        ['query'],
      ),
    ...
    'sto_getAccountProOrders' : IDL.Func(
        [Address],
        [IDL.Vec(STOrder)],
        ['query'],
      ),
    ...
    'sto_getStratOrderByTxid' : IDL.Func(
        [Txid__1],
        [IDL.Opt(STOrder)],
        ['query'],
      ),
        ...
        [
          ...
          IDL.Variant({
            ...
            'GridOrder' : IDL.Record({
              ...
              'spread' : IDL.Opt(
                IDL.Variant({ 'Geom' : Ppm, 'Arith' : Price__1 })
              ),
              'amount' : IDL.Opt(
                IDL.Variant({
                  'Percent' : IDL.Opt(Ppm),
    ...
    'withdraw2' : IDL.Func(
        ...
        [
          Amount,
          Amount,
          IDL.Variant({ 'Completed' : IDL.Null, 'Pending' : IDL.Null }),
        ],
        [],
      ),
